...
face_detection = YuNet(
    modelPath = "face_detection_yunet_2022mar.onnx",
...
# Smile evaluation
smile_evaluation = tflite.Interpreter(model_path = "model_32_lite")
...
while(not bCamFound):
    ...
    if(not bCamFound):
        if(idxCamera < CAMERA_MAX_IDX):
            ...
            logger.error("No camera found (CAMERA_MAX_IDX = {idx} reached)")
            sys.exit()
